...
for(k = 1:1:numTrials)
    ...
    cp_duration = round(cp_duration * Nfft) / Nfft;
    ...
    if(hard_bpf)
        ...
        xt_unfiltered = [xt_unfiltered zeros(1,512)];  % pad unfiltered signal to make it equal length for fft
    end
    ...
    average_case_error = mean([pi_average_case_error zero_average_case_error]);
    % disp("Average phase error")
    % disp(average_case_error)
    
    avpherr = avpherr + (average_case_error ./ numTrials);
    ...
    wcerr = max(wcerr, worst_case_error);
